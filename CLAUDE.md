...
- **SDK Shim 全量覆盖**：shim 覆盖 OpenClaw 全部 `plugin-sdk/*` 导出（手写 + 自动生成 stub）。手写模块受 `_handwritten.json` 清单保护，新增手写 shim 后 MUST 加入该清单（否则 `generate:sdk-shims` 会覆盖）。OpenClaw 更新时运行 `bun run generate:sdk-shims` 重新生成 stub
...
- **Shim 传播机制**：源码 shim 不直接到达运行时。传播链路：源码 → `build` 打包到 app bundle `plugin-bridge-sdk-shim/` → Bridge 启动前 integrity check → `install_sdk_shim()` 拷贝到插件 `node_modules/openclaw/`。**修改 shim 内容后 MUST bump 版本**（三处同步），否则 integrity check 判定版本一致而跳过重装，已安装插件永远拿不到新 shim。自动生成 stub 返回 `undefined`，如果插件对返回值做解构会 TypeError 崩溃 — 这是需要手写 shim 的典型信号

...
**多实例 Sidecar 架构**：每个 Chat Tab 拥有独立的 Bun Sidecar 进程，Settings/Launcher 使用 Global Sidecar，IM Bot 通过 Rust 层直接管理 Sidecar。
...
│  │ TabProvider │  │ TabProvider │  │  Launcher   │  │  聊天机器人   │ │
│  │ Tab Sidecar │  │ Tab Sidecar │  │ Global API  │  │  多 Bot 管理  │ │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └──────┬───────┘ │
│         │                │                │                │          │
├─────────┼────────────────┼────────────────┼────────────────┼──────────┤
│         ▼                ▼                ▼                ▼ Rust     │
│  ┌─────────────┐  ┌─────────────┐  ┌───────────┐  ┌───────────────┐ │
│  │ Tab Sidecar │  │ Tab Sidecar │  │  Global   │  │ ManagedImBots │ │
│  │ :31415      │  │ :31416      │  │  Sidecar  │  │ HashMap<botId,│ │
│  └─────────────┘  └─────────────┘  └───────────┘  │  Instance>    │ │
│                                                    └───────┬───────┘ │
│                                                            ▼         │
│                                                   Telegram Bot API   │
...
**Sidecar Owner 模型**：Tab、CronTask、BackgroundCompletion、ImBot 四种 Owner 共享 SidecarManager，所有 Owner 释放后 Sidecar 才停止。IM Bot 通过 `SidecarOwner::ImBot(session_key)` 复用同一套 Sidecar 基础设施。
...
IM 组件使用 `configService` 的专用函数（`addOrUpdateImBotConfig`、`updateImBotConfig`、`removeImBotConfig`）写盘，写盘后必须调用 `refreshConfig()` 同步 React 状态：

```typescript
// ✅ 正确：写盘 + 刷新 React state
const saveBotField = useCallback(async (updates) => {
    await updateImBotConfig(botId, updates);   // disk-first
    await refreshConfig();                      // sync React
}, [botId, refreshConfig]);

// ❌ 错误：只写盘不刷新，列表页/详情页看到过期数据
await updateImBotConfig(botId, updates);
// React config state 仍然是旧的！
```

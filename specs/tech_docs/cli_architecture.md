...
│   → 找到 bundled Bun + ~/.myagents/bin/myagents                      │
...
部分能力（Task / CronTask / Plugin）在 Rust Management API 而非 Bun。Admin handler 作为薄转发层，并通过 `wrapMgmtResponse()` / `mgmtError()` 保证：
...
**为什么 pre-flight 放在 Bun 而不是 Rust**：Bun 有一手的 `RuntimeFactory.detect()` / `queryModels()` 接口，而且 Bun 能给出带 `recoveryCommand` 的结构化错误；Rust 侧只能返回 opaque serde 错误。
...
| `Management API not available` | Bun Sidecar 起来了但 Rust Management API 没起 — CLI 会附带 `→ Run: myagents status` 指引 |

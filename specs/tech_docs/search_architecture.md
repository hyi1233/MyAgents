...
**仅 Tauri 可用**：搜索直接走 Tauri IPC（`invoke('cmd_search_*')`）到 Rust 层，不经 Bun Sidecar。浏览器开发模式（`start_dev.sh`）没有 fallback — UI 入口在非 Tauri 环境下不出现。
...
Session 文件由 Bun Sidecar 写入，索引在 Rust。两个显而易见的替代方案都有问题：

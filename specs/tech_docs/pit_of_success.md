...
- [Session watcher](#session-watcher) — 文件系统观察索引（v0.1.65）
...
**两次踩坑历史：**
- v0.1.65：`search/mod.rs::start_background_indexing`
- v0.1.70：`logger::init_buffered_writer`（commit `da8a08b` 后改为 lint enforcement）

---

<a id="session-watcher"></a>
## Session watcher (`src-tauri/src/search/session_watcher.rs`, v0.1.65)
...
- Phases: `cleanup / migration / skill-seed / agent-browser-setup / socks-bridge / sdk-init / external-runtime-restore`
...
<a id="fs-utils"></a>
## `fs-utils` (`src/server/utils/fs-utils.ts`, v0.1.69+)
...
**History.** v0.1.69 引入时同时处理两个问题：
- ① Bun-on-Windows `mkdirSync` EEXIST bug（v0.2.0 切 Node 后已消除）
- ② `Dirent.isDirectory()` 在 junction / symlink-to-dir 上返回 false（仍存在，helper 继续兜底）

---

<a id="subprocess"></a>
## `subprocess` (`src/server/utils/subprocess.ts`, v0.2.0+)

**Problem.** `Bun.spawn` → Node `child_process.spawn` 切换时遇到 stream 形态差异：`exited` Promise 触发时机、stdin 背压、stdout 是否 cached Web Stream 等。

**Surface.** `Bun.spawn` 兼容 adapter：
...
**Invariants enforced.** Caller 从 Bun API 平移到 Node 时不用每处重写 stream-shape 差异。

---

<a id="file-response"></a>
## `file-response` (`src/server/utils/file-response.ts`, v0.2.0+)

**Problem.** `new Response(Bun.file(p))` 在 Node 不存在等价物，每个 HTTP 路由返回文件都要内联 `fs.readFile + new Response`。
...
<a id="legacy-cas"></a>
## Legacy CronTask CAS Upgrade (`legacy_upgrade.rs`, v0.1.69+)

**Problem.** v0.1.69 之前的独立 CronTask 在首次加载时被检测为 "legacy"，需要自动升级成带 Task 的结构。多 Sidecar 启动时并发跑同一升级路径 → 重复创建 Task。

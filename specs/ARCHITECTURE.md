...
**Rust Management API (v0.1.69)**：CLI / 前端 / Bun 内部工具三条入口都需要 CRUD 定时任务、任务、想法这类**直接持久化在 Rust 的状态**。为避免每条入口都经前端 IPC 再转 Bun 再转 Rust，`src-tauri/src/management_api.rs` 在 app 启动时监听 `127.0.0.1:${随机端口}`（axum）直接暴露 HTTP 路由：

| 前缀 | 职责 | 典型调用方 |
|------|------|-----------|
| `/api/cron/*`（9 条） | CronTask CRUD + 调度控制 | CLI（`myagents cron`）、Bun `im-cron-tool.ts` |
| `/api/task/*`（13 条） | Task Center 任务 CRUD + run/rerun + doc 读写 | CLI（`myagents task`）、Bun `admin-api.ts` |
| `/api/thought/*`（2 条） | 想法 create / list | CLI（`myagents thought`）、Bun `admin-api.ts` |
| `/api/im/*` + `/api/im-bridge/*` | IM Bot 唤醒 + 媒体下发 + Plugin Bridge 回调 | Bun / 社区插件 Bridge |
| `/api/plugin/*`（3 条） | OpenClaw 插件 CRUD | CLI |
| `/api/agent/runtime-status` | Agent 运行时状态查询 | Bun / 前端 |

端口号通过 `MYAGENTS_MANAGEMENT_PORT` 环境变量注入到 Bun Sidecar 进程。这是项目内**唯一**的"Bun → Rust"反向 HTTP 通道，规避了"所有前端 HTTP 走 Rust proxy → Bun"这条主流向对后端间通信的不适配。所有客户端必须走 `crate::local_http::builder()`（loopback，但仍复用 no_proxy 保护）。
...
**门控链路**：Rust `sidecar.rs` 在启动 Sidecar 时读取 `config.multiAgentRuntime` + `agent.runtime` → 注入 `MYAGENTS_RUNTIME` 环境变量 → Bun `factory.ts` 读取 → `shouldUseExternalRuntime()` 分流请求到 `external-session.ts`。前端 `Chat.tsx` 用同样的门控决定 `currentRuntime`，源头门控确保所有下游派生自动安全。

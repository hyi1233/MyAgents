...
export function restoreExternalSessionState(
  ...
  // CC: session ID === our session ID (CC uses our --session-id).
  // Codex: thread ID is different (generated by Codex), persisted as runtimeSessionId.
  const meta = getSessionMetadata(sessionId);
  lastRuntimeSessionId = meta?.runtimeSessionId || sessionId;

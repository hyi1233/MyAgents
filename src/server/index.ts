      ...
      if (pathname === '/api/im/chat' && request.method === 'POST') {
        try {
          const payload = (await request.json()) as {
            ...
            botId?: string;
            botName?: string;
            // Pattern A — Per-Request Identity (IM Pipeline v2). Generated by Rust at the
            // IM edge (mod.rs spawn entry). Threaded through enqueueUserMessage →
            // messageQueue → SDK turn for full-chain trace.
            requestId?: string;
          ...
          if (shouldUseExternalRuntime()) {
            ...
            const result = await enqueueUserMessage(
              ...
              metadata,
              payload.requestId ?? undefined, // Pattern A — IM trace ID (forwarded from Rust)
            );
